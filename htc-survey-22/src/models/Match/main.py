...
# --reg 1 -d config/Match/old/WOS.yaml -m config/Match/old/model.yaml --mode train
...
def main(data_cnf, model_cnf, mode, reg):
    ...
    # else:
    labels_all = [d["labels"] for d in df]
    labels = [ls[-1] for ls in labels_all]
    ...
    if mode is None or mode == 'train':
        ...
        for train_index, test_index in splitter.split(tickets_tokenized, labels):
            fold_i += 1
            ...
            x_train, x_test = tickets_tokenized.iloc[train_index], tickets_tokenized.iloc[test_index]

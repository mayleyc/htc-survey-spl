...
from utils.general import apply_filters
...
class Extractor():
    ...
    def __init__(self, id, revid, urlbase, title, page):
        ...
        self.__extract_fun: Callable[[str], List[str]] = None
    ...
    TOPICS_CLEAN = [lambda t: re.sub(r"<([^>]+)>", "", t),  # strip HTML tags
                    lambda t: re.sub(r"([%s])+" % re.escape(string.punctuation), " ", t),  # strip punctuation
                    lambda t: " ".join(t.split()),  # remove multiple whitespace
                    str.lower]

    @property
    def extract_raw_topics(self) -> Callable[[str, str], List[str]]:
        """
        Depending on the lenaguage, build the right extraction function for topics
        
        :return: a function that takes a wikipedia page and extract the list of portals (topics) found within
        """
        if self.__extract_fun is None:
            m = re.search(r".+://([a-z]{2})\..+\.org", self.url)
            lang = m.group(1)

            fun: Callable[[str], List[str]]
            if lang == "it":
                def parse_it(x: str, *args) -> List[str]:
                    ts = re.findall(r"{{\s*portale\s*\|([^{}]+)}}", x, re.I | re.S)
                    if ts:
                        ts = [t.strip() for s in ts for t in s.split("|")]
                    return ts if ts else list()

                fun = parse_it
            elif lang == "fr":
                def parse_fr(x: str, *args) -> List[str]:
                    ts = re.findall(r"{{\s*portail\s*\|([^{}]+)}}", x, re.I | re.S)
                    if ts:
                        ts = [t.strip() for s in ts for t in s.split("|")]
                    return ts if ts else list()
                ...
                def parse_en(x: str, page: str) -> List[str]:
                    ts = set()
                    ts1 = re.findall(r"{{\s*portal\s*\|([^{}]+)}}", x, re.I | re.S)
                    if ts1:
                        ts = {t.strip() for s in ts1 for t in s.split("|")}

                    ts2 = re.findall(r"{{\s*portal\s+bar\s*\|([^{}]+)}}", x, re.I | re.S)
                    if ts2:
                        ts |= {t.strip() for s in ts2 for t in s.split("|")}

                    ts3 = re.findall(r"{{\s*subject\s+bar\s*\|([^{}]+)}}", page, re.I | re.S)
                    if ts3:
                        candidates = [t.strip() for s in ts3 for t in s.split("|")]
                        matches = {re.search(r"portal\s*\d*\s*=\s*(\w+(?:\s*\w+)*)", c, re.I) for c in candidates}
                        ts |= {e.group(1).strip() for e in matches if e is not None}
                    return list(ts) if ts else list()

                fun = parse_en
            else:
                raise ValueError("Unsupported language")
            self.__extract_fun = fun
        return self.__extract_fun

    @staticmethod
    def process_str(matches: List[str], stop_set: Iterable[str] = frozenset()) -> List[str]:
        """
        Process a list of raw string candidate topics

        :param matches: keywords to __process
        :param stop_set: list of stopwords not to be included in returned words (case insensitive)
        :return: list of words extracted
        """

        topics = {apply_filters(re.sub(r"\d=(\w+)", r"\1", s.strip().strip("}")),
                                Extractor.TOPICS_CLEAN) for s in matches}
        topics_clean = {re.sub(r"(\s+)|(\t+)|(\n+)", "_", unquote(t)) for t in topics if t and t not in stop_set}
        return list(topics_clean)

    def get_topics(self) -> List[str]:
        """
        Get topics from article page

        :return: list of topics (if any)
        """
        matches = list()
        for sec in self.page:
            m: List[str] = self.extract_raw_topics(sec, " ".join(self.page))
            matches += m
        topics = self.process_str(matches) if matches else list()
        if self.id == "979661":
            # TODO: remove this, just for test
            print(self.url, self.title, topics)
        return topics
    ...
    def extract(self, out, html_safe=True):
        ...
        if self.to_json:
            ...
            if tops and json_data["text"]:
                # Only write articles with valid text and at least 1 topic
        ...
        if any(errs):
            logging.warning("Template errors in article '%s' (%s): title(%d) recursion(%d, %d, %d)", self.title,
                            self.id, *errs)

...
def _setup_training_BERT_SC(train_config, tokenizer, workers: int, data, labels, data_val, labels_val):
    # -------------------------------


    train_data = TransformerDatasetFlat(data, labels)
...
for c in config_list:
    ...
    for (x_train, y_train), (x_test, y_test) in ds_manager.get_split():
        batch_no = 0
        training_loader, validation_loader = _setup_training_BERT_SC(train_config=config, tokenizer=tokenizer,
                                                    workers=0,
        ...
        for loader in [training_loader, validation_loader]:
            ...
            filename = f"emb_{model_name}_{dataset}_{date_string}_{emb_type}_batch{batch_no}.pickle"
            ...
            with open(fp_dest, 'wb') as f:
                pickle.dump([all_embeddings, all_labels], f, protocol=pickle.HIGHEST_PROTOCOL)
               
        break

#
with open(fp_dest, "rb") as f:
    all_embeddings, all_labels = pickle.load(f)

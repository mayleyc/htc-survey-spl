...
def _train_single_split(x_train, x_test, y_train, y_test,
                        config: Dict, model_class: Type,
                        logits_fn, enc_: Path):
    # Create and train a model
    trainer, train_load, test_load = _setup_training(train_config=config, model_class=model_class,
                                                     workers=workers,
                                                     data=x_train, labels=y_train,
                                                     data_val=x_test, labels_val=y_test,
                                                     logits_fn=logits_fn,
                                                     enc_=enc_)
    trainer.train(train_load, None)
...
def run_configuration():
    ...
    config_list: List = ["bert_bgc.yml", "bert_wos.yml", "bert_rcv1.yml", "bert_bugs.yml", "bert_amz.yml"]

    ...
    def forward(self, batch: Tuple[Dict[str, torch.Tensor], torch.LongTensor], *args, **kwargs) -> Any:
        ...
        if self.lm.config.model_type == "bert":
            ...
            # "I'm an artist" tests combining min/max/mean
                ...
                layers_avg = [torch.cat([max_emb(e, mask), min_emb(e, mask)], dim=1) for e in hidden_states[-self.cfnl:]]
                ...
                layers_avg = [torch.cat([max_emb(e, mask), mean_emb(e, mask)], dim=1) for e in hidden_states[-self.cfnl:]]
                doc_emb = torch.stack(layers_avg, dim=0).mean(dim=0)  # (bs, 2*dim)
        ...
        return logits, labels, drop_doc_emb, doc_emb

...
class Trainer:
    ...
    def __init__(self, model: TrainableModel, train_params: Dict[str, Any],
        ...
        self.model_folder = Path(f"{train_params['MODEL_FOLDER']}_{self._model_start_time}")
        self.__save_train_specs(train_params)
        # Generic parameters
        self.device: torch.device = torch.device(train_params["DEVICE"])
        self.__epochs: int = train_params["EPOCHS"]
        self.__start_epoch: int = 0
        self.__log_every_batches: int = 100
    ...
    def __save_train_specs(self, train_params):
    ...
    def data_unpack(self, data_batch) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        If 'unpack_flag' is set it assumes every dataset sample can be unpacked in X, y, which are respectively passed to the model and the loss/metric functions
    ...
    def train_one_epoch(self, training_loader: td.DataLoader, epoch_num: int):
        ...
        for i, data in enumerate(training_loader):
            ...
            loss_args = self.model.filter_data_loss(outputs, y, device=self.device) \
                if y is not None else self.model.filter_data_loss(outputs, device=self.device)
            loss = self._loss_fn(**loss_args) if isinstance(loss_args, dict) else self._loss_fn(*loss_args)
            loss.backward()
            self._optimizer.step()
            running_loss += loss.item()
            # ---------------------- REPORTING ------------------------
            if i % self.__log_every_batches == 0:
                last_loss: float = running_loss / self.__log_every_batches  # loss per batch
                running_loss = 0.
    ...
    def validate_one_epoch(self, validation_loader: td.DataLoader):
        ...
        for i, val_data in enumerate(validation_loader):
            ...
            # Loss computation
            loss_args = self.model.filter_data_loss(val_outputs, val_y, device=self.device) \
                if val_y is not None else self.model.filter_data_loss(val_outputs, device=self.device)
            val_loss = self._loss_fn(**loss_args) if isinstance(loss_args, dict) else self._loss_fn(*loss_args)
        ...
        metric_values = {k: v.cpu().item() for k, v in self._metrics.compute().items()}
        # Return all values
    ...
    def train(self, training_loader: td.DataLoader, validation_loader: Optional[td.DataLoader] = None):
        self.__save_run_specs()
        for epoch_number in range(self.__start_epoch, self.__epochs):
            print(f"--- EPOCH {epoch_number + 1} / {self.__epochs} ---")
            # ---------------------- UPDATE ------------------------
            # -- TRAINING --
            ...
            if validation_loader is not None:
                ...
                with torch.no_grad():
                    avg_val_loss, metric_values = self.validate_one_epoch(validation_loader)
            # ---------------------- REPORTING ------------------------
            # -- Write to board ---
            if self.board:
            ...
            if validation_loader is not None:
                print(f"              || {'Val loss':<10}: {old_val_loss_value_print:>10.4f} {'->':^5} {avg_val_loss:>4.4f}")
            ...
            has_improved: bool = self.early_stopper(es_report) if validation_loader is not None else True
            ...
            if self.early_stopper.exit_flag:
                break
    ...
    def save_epoch_checkpoint(self, epoch: int, metrics: Dict[str, float]):
        ...
        if current_dump_folder.is_dir() and current_dump_folder.exists():
            ...
            logging.warning(
                f"Dump folder '{current_dump_folder}' already exists. You are probably overwriting an existing epoch dump. The old one will be renamed in {new_name}")
        ...
        epoch_state_data = dict(epoch_number=epoch, metrics=metrics, best_score=self.early_stopper.best_score)
        ...
        epoch_folders_list = [x[0] for x in os.walk(self.model_folder)][1:]
        if len(epoch_folders_list) > self.epochs_to_keep:
            ...
            # Sanity check
            assert len([x[0] for x in os.walk(self.model_folder)][1:]) <= self.epochs_to_keep

    def load_previous(self, reload_path: str, model_only: bool = False) -> Path:
        """
        ...
        self.__start_epoch = data["epoch_number"] + 1
